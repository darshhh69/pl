...
// Initialize cart if not already set (copied from ourproducts.php)
if (!isset($_SESSION['cart'])) {
    $_SESSION['cart'] = [];
}

// Calculate total items in cart for display (copied from ourproducts.php)
$cart_item_count = 0;
foreach ($_SESSION['cart'] as $item) {
    $cart_item_count += $item['quantity'];
}

// Note: Add to Cart/Remove from Cart logic is not needed on index.php itself,
...
<body>
  ...
  <main class="container">
    <section class="hero" aria-label="Page introduction">
      ...
      <a href="index.php" class="btn-primary" role="button" aria-label="Return to homepage">Back to Home</a>
    </section>
